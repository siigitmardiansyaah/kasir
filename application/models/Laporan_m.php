...
class Laporan_m extends CI_Model {
	...
	{
        $query = $this->db->query("SELECT DATE_FORMAT(tanggal,'%d-%m-%Y') as tanggal,nota, IFNULL(total_bayar,0) as sub_total, IFNULL(diskon,0) as diskon, IFNULL(total_bayar,0) as total, IFNULL(jumlah_uang,0) as pembayaran from transaksi where DATE_FORMAT(tanggal,'%Y-%m-%d') between '$tgl1' AND '$tgl2' order by tanggal asc");
        return $query->result();
	}
